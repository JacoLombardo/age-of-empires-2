...
function Chat({ unit, civilization, structure, technology }) {
  ...
  const getComments = async () => {
    if (unit) {
      const q = query(
        collection(db, "Comments", "Units", `Unit id-${unit.id}`),
        orderBy("date")
      );
      ...
      <div className="commentDiv">
        {chatComments &&
          chatComments.map((comment, index) => {
            if (unit) {
              return <Comment key={index} comment={comment} unit={unit} />;
            }
            if (civilization) {
              return (
                <Comment
            ...
            if (structure) {
              return (
                <Comment key={index} comment={comment} structure={structure} />
              );
            }
            if (technology) {
              return (
